...
var _ = Describe("sending a simple trace to Tempo through the OpenTelemetry Collector", Ordered, Serial, Label("docker", "integration", "slow", "example"), func() {
	BeforeAll(func() {
		...
		traceEndpoint, err := localTempoEndpoint.OTLPTraceEndpoint(ctx)
