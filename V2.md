...
- Full-fidelity `oats migrate` for fixture/input semantics. A best-effort
  ...
  fields (matrix/custom-checks still need manual follow-up).
...
multi-file compose/env and kubernetes→k3d mappings. The v1 binary (`oats`)
and the v2 binary (`oats-v2`) still read different file shapes.
